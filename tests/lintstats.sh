...
PYLINT_SCORE=$( pipenv run pylint compiler | tail -2 | grep -Eo '[0-9\.]+/10' | tail -1 | sed s/\\/10// )
...
# Static checks.
MYPY_STATUS=$( pipenv run mypy -p compiler | grep -v "test.*" | wc -l | tr -d '[:space:]' )
...
pipenv run pydocstyle --convention=numpy --add-ignore=D401 compiler
PYDOCSTYLE_STATUS=$?
